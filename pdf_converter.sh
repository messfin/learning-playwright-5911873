...
# FusionWW Framework Documentation to PDF Converter
...
echo -e "${GREEN}FusionWW Documentation PDF Converter${NC}"
echo "========================================"
echo ""
...
if ! command -v pandoc &> /dev/null; then
    ...
    echo "  Ubuntu:  sudo apt-get install texlive"
    echo "  Windows: choco install miktex"
    exit 1
fi

# Input and output files
INPUT_FILE="documentation/fusionww-framework.md"
OUTPUT_FILE="documentation/FusionWW-Framework-Documentation.pdf"

# Check if input file exists
if [ ! -f "$INPUT_FILE" ]; then
    echo -e "${YELLOW}Input file not found. Creating it now...${NC}"
    mkdir -p documentation
    
    # Create the markdown file with full documentation
    cat > "$INPUT_FILE" << 'EOF'
...
EOF
    ...
    echo -e "${GREEN}✓ Markdown file created: $INPUT_FILE${NC}"
fi
...
pandoc "$INPUT_FILE" \
    ...
    --metadata title="FusionWW Playwright Framework Documentation" \
    --metadata author="QA Automation Team" \
    --metadata date="$(date '+%B %Y')" 2>&1
...
if [ $? -eq 0 ]; then
    ...
    # Try to open the PDF (platform-dependent)
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS
        echo -e "${YELLOW}Opening PDF in default viewer...${NC}"
        open "$OUTPUT_FILE"
    ...
    echo "Please check that LaTeX is properly installed."
    exit 1
fi
